...
function publicUrl(handle: string): string {
  const base =
    process.env.NEXTAUTH_URL ??
    process.env.AUTH_URL ??
    "http://localhost:3000";
  return new URL(`/${handle}`, base).href;
}
...
export async function generateMetadata({
  ...
  const showcase = await getPublishedShowcaseByHandle(normalizedHandle);
  ...
  return metadataForShowcase(showcase, publicUrl(normalizedHandle));
}
...
export default async function PublicShowcasePage({
  ...
  const { handle } = await params;
  const normalizedHandle = assertValidHandle(handle);
  const showcase = await getCachedPublishedShowcaseByHandle(normalizedHandle);

  if (!showcase) {
    notFound();
  }

  const shareUrl = publicUrl(normalizedHandle);
